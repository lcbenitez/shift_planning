...
class WorkRole(models.Model):
    ...
    def _compute_available_employees_count(self):
        ...
        for role in self:
            if role.skill_ids:
                ...
                employees_with_skills = self.env['hr.employee'].search([
                    ('skill_ids', 'in', role.skill_ids.ids),
                    ('active', '=', True)
                ])
                
                # Filtrar empleados que tengan TODAS las habilidades (no solo algunas)
                qualified_employees = employees_with_skills.filtered(
    ...
    def action_view_qualified_employees(self):
        ...
        if not self.skill_ids:
            # Si no requiere habilidades específicas, mostrar todos los empleados activos
            domain = [('active', '=', True)]
        else:
